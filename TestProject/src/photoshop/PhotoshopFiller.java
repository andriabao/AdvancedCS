...
public class PhotoshopFiller extends Component {
    ...
    public void brighten(int amount) {
        ...
        for(int i = 0; i < h; i++) {
        	for(int j = 0; j < w; j++) {
        		Color c = new Color(pixels[i][j].getRGB());
...
//        		for(int j = 0; j < w/2; j++) {
//        			Color c = new Color(pixels[i][j].getRGB());
//        			pixels[i][j] = new Color(pixels[i][w-j].getRGB());
//        			pixels[i][w-j] = c;
//        		}
//        	}
//        } else {
...
//        		for(int j = 0; j < w; j++) {
//        			Color c = new Color(pixels[i][j].getRGB());
//        			pixels[i][j] = new Color(pixels[i][h-i].getRGB());
//        			pixels[i][w-j] = c;
//        		}
//        	}
//        }
    ...
    public void simplify() {
    		...
    		Color[] colorList = {Color.BLUE, Color.RED,Color.ORANGE, Color.MAGENTA,
                Color.BLACK, Color.GREEN, Color.YELLOW, Color.CYAN};
        ...
        for(int i = 0; i < h; i++) {
        	for(int j = 0; j < w; j++) {
        		...
        		int min = 0;
        		Color myColor = new Color(pixels[i][j].getRGB()); 
        		
        		for(int a = 1; a < colorList.length; a++) {
        			if(Math.pow(colorList[a].getRed()-myColor.getRed(), 2) + 
        					Math.pow(colorList[a].getGreen()-myColor.getGreen(), 2) + 
        					Math.pow(colorList[a].getBlue()-myColor.getBlue(), 2) < 
        					Math.pow(colorList[min].getRed()-myColor.getRed(), 2) + 
        					Math.pow(colorList[min].getGreen()-myColor.getGreen(), 2) + 
        					Math.pow(colorList[min].getBlue()-myColor.getBlue(), 2)) {
        				min = a;
        			}
        		}
        		pixels[i][j] = colorList[min];
        	}
        }
        // your code here
    }
